...
- [ ] T001 [Setup] `backend/card` 앱 생성 및 `INSTALLED_APPS` 등록 (앱 폴더 확인 후 등록)
- [ ] T002 [Setup] `backend/card/urls.py` 생성 및 프로젝트 `config/urls.py`에 연결
- [ ] T003 [Setup] Celery 및 Redis 설정 확인 (`backend/config/settings.py` 점검)
...
- [ ] T005 [Base] `backend/card/models.py`에 `GithubEvent` 모델 구현 (증분 수집용, Event ID PK)
- [ ] T006 [Base] `backend/card/models.py`에 `AnalysisResult` 모델 구현
- [ ] T007 [Base] `backend/card/admin.py`에 모델 등록
- [ ] T008 [Base] 데이터베이스 마이그레이션 생성 및 적용 (`makemigrations`, `migrate`)
...
- [ ] T009 [US1] `backend/templates/card/placeholder.svg` 생성 (분석 중 이미지)
- [ ] T010 [US1] `backend/card/views.py`에 `CardView` 기본 로직 구현 (Cold Miss 감지 및 Placeholder 반환)
- [ ] T011 [P] [US1] `backend/card/services.py`에 `GitHubClient` 구현 (이벤트 ID 파싱 포함)
- [ ] T012 [P] [US1] `backend/card/services.py`에 `GeminiClient` 구현 (프롬프트 관리 포함)
- [ ] T013 [US1] `backend/card/tasks.py`에 `analyze_user_task` 구현 (GitHub 이벤트 증분 수집 -> DB 저장 -> 최근 24시간 조회 -> Gemini 분석 -> 결과 저장)
- [ ] T014 [US1] `backend/card/views.py`에서 Cold Miss 시 `analyze_user_task` 호출 연결

## 4단계: 사용자 스토리 2 - 분석 완료 후 카드 조회 (Hit & Fresh)

**목표**: 분석이 완료된 사용자에 대해 실제 결과가 반영된 SVG 카드를 반환합니다.

- [ ] T015 [US2] `backend/templates/card/card.svg` 생성 (데이터 바인딩용 템플릿)
...
- [ ] T019 [US3] `backend/card/tasks.py`에 Redis Distributed Lock 구현 (중복 실행 방지)

## 최종 단계: 다듬기 및 교차 관심사

**목표**: 에러 처리, 코드 정리 및 최종 점검.

- [ ] T020 [Final] `backend/templates/card/error.svg` 생성
- [ ] T021 [Final] `backend/card/views.py`에 예외 처리 및 Fallback SVG 적용
- [ ] T022 [Final] `backend/card/tests.py`에 통합 테스트 작성 (Cold/Fresh/Stale 시나리오, 응답 시간, 증분 수집 검증)
